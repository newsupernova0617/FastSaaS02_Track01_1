...
export function SearchScreen(props: { result: QueryObserverResult<SourcedData<SearchResponse>, Error>; onSubmitSearch: () => void }) {
        ...
        <div class="mt-4">
          <div class="rounded-[28px] bg-white p-5 shadow-sm">
            <div class="flex items-center justify-between gap-3">
              <div class="text-[11px] font-semibold text-slate-500">{props.result.data.data.period}</div>
            ...
            <div class="mt-1 text-3xl font-black text-slate-950">
              {currency.format(props.result.data.data.total)}
              <span class="ml-1 text-base font-semibold">원</span>
            </div>
            <div class="mt-2 text-sm text-slate-600">
              하루 평균 <span class="font-bold text-slate-950">{currency.format(props.result.data.data.averagePerDay)}원</span>
            </div>

            <div class="mt-4 h-px bg-slate-200"></div>

            <div class="mt-4 space-y-2">
              {props.result.data.data.highlights.map((highlight) => (
          ...
          <div class="mt-4 rounded-3xl border border-dashed border-slate-300 bg-white/70 p-4 text-sm text-slate-600">
            ...
            {props.result.data.data.insight}
          </div>
        </div>
      )}
    </div>
