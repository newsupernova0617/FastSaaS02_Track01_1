# FastSaaS Backend 코드 기준 아키텍처 가이드

이 문서는 `FastSaaS02_Track01_1/backend`의 현재 실제 소스코드를 기준으로 백엔드 구조를 정리한 문서입니다.

- 추상 설계가 아니라 현재 존재하는 파일과 경로를 기준으로 설명합니다.
- 기준 소스: `src/index.ts`, `src/routes/*`, `src/services/*`, `src/db/*`, `src/middleware/*`, `tests/*`
- 상세 폴더 트리는 `docs/backend_structure.md`에서 따로 봅니다.
